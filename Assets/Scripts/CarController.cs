...
public class CarController : MonoBehaviour
{
    //CAR SETTINGS
      ...
      private int maxReverseSpeed = 45; //The maximum speed that the car can reach while going on reverse in km/h.
      // [Range(1, 10)]
      ...
      [Space(10)]
      private Vector3 bodyMassCenter; // This is a vector that contains the center of mass of the car. I recommend to set this value
                                    // in the points x = 0 and z = 0 of your car. You can select the value that you want in the y axis,
                                    // however, you must notice that the higher this value is, the more unstable the car becomes.
                                    // Usually the y value goes from 0 to 1.5.
      public TrailRenderer[] Tyremarks;

      public VehiclesPreset preset = null;
      ...
      */
      ...
      [Space(10)]
      public GameObject backLeftMesh;
      public WheelCollider backLeftCollider;
      [Space(10)]
      public GameObject backRightMesh;
    ...
    void AnimateWheelMeshes(){
      try{
        ...
        Vector3 BLWPosition;
        backLeftCollider.GetWorldPose(out BLWPosition, out BLWRotation);
        backLeftMesh.transform.position = BLWPosition;
        backLeftMesh.transform.rotation = BLWRotation;

        Quaternion BRWRotation;
        Vector3 BRWPosition;
    ...
    public void GoForward(){
      ...
      if(localVelocityZ < -1f){
        ...
        if(Mathf.RoundToInt(carSpeed) < this.preset.maxSpeed){
          //Apply positive torque in all wheels to go forward if maxSpeed has not been reached.
          frontLeftCollider.brakeTorque = 0;
          frontLeftCollider.motorTorque = (this.preset.accelerationMultiplier * 50f) * throttleAxis;
          frontRightCollider.brakeTorque = 0;
          frontRightCollider.motorTorque = (this.preset.accelerationMultiplier * 50f) * throttleAxis;
          backLeftCollider.brakeTorque = 0;
          backLeftCollider.motorTorque = (this.preset.accelerationMultiplier * 50f) * throttleAxis;
    ...
    public void Handbrake(){
      ...
      float secureStartingPoint = driftingAxis * FLWextremumSlip * this.preset.handbrakeDriftMultiplier;

      if(secureStartingPoint < FLWextremumSlip){
        driftingAxis = FLWextremumSlip / (FLWextremumSlip * this.preset.handbrakeDriftMultiplier);
      }
      if(driftingAxis > 1f){
        driftingAxis = 1f;
      }
      ...
      // = 1f.
      if(driftingAxis < 1f){
        FLwheelFriction.extremumSlip = FLWextremumSlip * this.preset.handbrakeDriftMultiplier * driftingAxis;
        frontLeftCollider.sidewaysFriction = FLwheelFriction;

        FRwheelFriction.extremumSlip = FRWextremumSlip * this.preset.handbrakeDriftMultiplier * driftingAxis;
        frontRightCollider.sidewaysFriction = FRwheelFriction;

        RLwheelFriction.extremumSlip = RLWextremumSlip * this.preset.handbrakeDriftMultiplier * driftingAxis;
        backLeftCollider.sidewaysFriction = RLwheelFriction;

        RRwheelFriction.extremumSlip = RRWextremumSlip * this.preset.handbrakeDriftMultiplier * driftingAxis;
        backRightCollider.sidewaysFriction = RRwheelFriction;
      }
